	...
	if (gameplay_data.play_date && gameplay_data.scores) {
		...
		.then(function(result) {
			...
			.then(function(gameplay) {
				gameplay.save()
				.then(function(gameplay) {
					gameplay.reload({ include: [{model: models.Game}, {model: models.GameplayScore, as: 'Scores', include: [{model: models.User, as: 'Player'}] }] })
						.then(function(scores) { res.jsonp(gameplay); });
				});
			});
		})
